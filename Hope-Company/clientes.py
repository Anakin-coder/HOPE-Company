...
def listar_produtos():
    with closing(conectar()) as con, closing(con.cursor()) as cur:
        cur.execute("ELECT p.id_produto, p.descricao, p.id_cliente, p.quantidade, p.preco, c.nome, c.sexo FROM produto p INNER JOIN cliente c ON p.id_cliente = c.id_cliente ORDER BY p.id_produto")
        return rows_to_dict(cur.description, cur.fetchall())
